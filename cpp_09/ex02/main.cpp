...
/*   Updated: 2023/09/29 19:55:35 by tmichel-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char **av)
{
	...
	for (int i = 1; i < ac; ++i)
	{
		d_numbers.push_back(std::atoi(av[i]));
		l_numbers.push_back(std::atoi(av[i]));
		std::cout << av[i] << " ";
	}
	std::cout << std::endl;
	PmergeMe<std::deque<int>> d_sorter;
	std::deque<int> sortedList = d_sorter.sort(d_numbers);
	PmergeMe<std::list<int>> l_sorter;
	std::list<int> sortedList2 = l_sorter.sort(l_numbers);
	std::cout << "After: ";
